...
## Technical Details
- **Physics**: N-body gravitational simulation. The ship is attracted by every body, and the bodies attract each other; the Sun is pinned to the origin to prevent whole-system drift. `dt` is clamped to 0.05s. Simple collision response pushes the ship to a body's surface and damps/reflects relative velocity; a soft (low relative-velocity) contact counts as a landing.

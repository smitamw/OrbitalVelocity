...
- **UI**: Custom OpenGL-rendered controls (rendered in a separate aspect-corrected UI pass):
    ...
    - **Pinch-to-Zoom**: Its range (`MIN_ZOOM` and `MAX_ZOOM_FACTOR` in `Renderer.cpp`) reaches from a close ship view out far enough to frame Neptune's orbit, so the entire solar system is viewable.
